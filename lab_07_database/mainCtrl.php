...
getRouter()->setDefaultRoute('showConverter'); // akcja/ścieżka domyślna
getRouter()->setLoginRoute('doLogin'); // akcja/ścieżka na potrzeby logowania (przekierowanie, gdy nie ma dostępu)

getRouter()->addRoute('showConverter',    'ConverterCtrl',  ['user','admin']);
getRouter()->addRoute('compute',           'ConverterCtrl',  ['user','admin']);
getRouter()->addRoute('doLogin',       'LoginCtrl');
